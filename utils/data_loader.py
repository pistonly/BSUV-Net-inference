...
def read_half_image(path):
    ...
    h_valid = int(h / 16) * 16
    w_valid = int(w / 16) * 16
    return True, img[:h_valid, :w_valid]

class imageFoldLoader(data.IterableDataset):
    def __init__(self, img_dir, empty_bg="no", empty_win_len=0, empty_bg_path="",
                 recent_bg=0, recent_bg_opp="mean", seg_network=None,
                 transforms_pre=None, transforms_post=None):
        """
        Initialization of video loader
        Args:
            ...
            transforms_pre (torchvision.transforms): Transforms to be applied to each input after converting to tensors
        """
        img_list = [str(f) for f in Path(img_dir).iterdir() if f.suffix.lower() in ['.jpg', '.png', '.bmp']]
        img_list.sort()
        ...
        ret, fr = read_half_image(img_list[0])
        if self.recent_bg:
            self.recent_bg_sum = np.zeros_like(fr).astype(np.float)
    ...
    def __readRGB(self, path):
        assert os.path.exists(path), "{} does not exist".format(path)
        im = cv2.cvtColor(read_half_image(path)[1], cv2.COLOR_BGR2RGB).astype(np.float)/255
        h, w, _ = im.shape
        h_valid = int(h / 16) * 16
        w_valid = int(w / 16) * 16
        return im[:h_valid, :w_valid, :]
